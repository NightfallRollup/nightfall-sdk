...
import poseidonConstants from "./poseidon-constants";
import { addMod, mulMod, powerMod } from "./number-theory";

const { C, M, SNARK_SCALAR_FIELD: q } = poseidonConstants;
const { generalise } = gen;

function ark(state: any, c: any, it: any) {
...
function sbox(state: any, f: any, p: any, r: any) {
  const N = state.length;
...
function poseidonHash(_inputs: any) {
  ...
  const inputs = _inputs.map((i: any) => BigInt(i.field(q)));
